...
def _run_one(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    memory_methods = frozenset({"context_memory_B25", "random_memory_B25"})
    ...
    if generator.calls != mandatory_bootstrap_calls + post_generations:
    ...
    generation_cap_satisfied = (
        post_generations <= registered_budget
        if registered_budget is not None
        else True
    )
    ...
    result = {
        ...
        "budget": {
            ...
            "budget_semantics": (
                "at_most_switch_and_generator_on_random_B25_opportunities"
