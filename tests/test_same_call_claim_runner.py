...
SCRIPT_PATH = WORKSPACE / "scripts" / "run_same_call_confirmation_v1.py"
...
class RegisteredBudgetTests(unittest.TestCase):
    def test_future_suffix_permutation_preserves_prefix_release_times_and_types(self) -> None:
        tape = [
            [[0, 0], [1, 4], [2, 8], [3, 12], [4, 16], [5, 20]],
            [[1, 0], [0, 4], [3, 8], [2, 12], [5, 16], [4, 20]],
        ]
        grid_types = ("W", "E", "W", "E", "W", "E")
        original = json.loads(json.dumps(tape))
        changed = SCRIPT._permute_future_suffix(
            tape,
        ...
        for before_agent, after_agent in zip(original, tape):
            for before, after in zip(before_agent, after_agent):
                self.assertEqual(before[1], after[1])
                if before[1] < 8:
                    self.assertEqual(before, after)
                else:
                    self.assertEqual(
                        grid_types[before[0]], grid_types[after[0]]
                    )
    ...
    def test_all_requested_method_names_are_stable(self) -> None:
        ...
        self.assertEqual(len(AVAILABLE_METHODS), 21)
        ...
        self.assertTrue({
            ...
            JS_CAP_G5_METHOD,
            CONTEXT_NO_REACTIVATION_METHOD,
        }.issubset(AVAILABLE_METHODS))
        self.assertIn("proposed_cohort_B25", REGISTERED_METHODS)
        self.assertIn("uniform", REGISTERED_METHODS)

    def test_same_call_budget_contracts_and_even_schedules(self) -> None:
        self.assertEqual(G4_POST_BOOTSTRAP_CAP, 4)
        self.assertEqual(G5_POST_BOOTSTRAP_CAP, 5)
        expected = {
            EXACT_EVEN_G4_METHOD: (4, [25, 50, 75, 99]),
            EXACT_EVEN_G5_METHOD: (5, [20, 40, 60, 80, 99]),
            RANDOM_G5_METHOD: (5, None),
        }
        for method, (budget, schedule) in expected.items():
            self.assertIn(method, EXACT_BUDGET_METHODS)
            self.assertEqual(method_budget(method, 100), budget)
            self.assertEqual(method_generation_cap(method, 100), budget)
            policy = make_exact_policy(
                method, num_scored_windows=100, policy_seed=17
            )
            assert policy is not None
            accepted = [
                decision_index
            ...
            policy.finalize()
            self.assertEqual(len(accepted), budget)
            if schedule is not None:
                self.assertEqual(accepted, schedule)
        self.assertEqual(method_budget(JS_CAP_G5_METHOD, 100), 5)
        self.assertEqual(method_generation_cap(JS_CAP_G5_METHOD, 100), 5)
        self.assertNotIn(JS_CAP_G5_METHOD, EXACT_BUDGET_METHODS)
        self.assertEqual(method_budget(CONTEXT_NO_REACTIVATION_METHOD, 100), 25)
        self.assertEqual(
            method_generation_cap(CONTEXT_NO_REACTIVATION_METHOD, 100), 25
        )
        self.assertNotIn(CONTEXT_NO_REACTIVATION_METHOD, EXACT_BUDGET_METHODS)
        self.assertIn(CONTEXT_NO_REACTIVATION_METHOD, SCORE_METHODS)

    def test_no_reactivation_ablation_preserves_remaining_priority_order(self) -> None:
        self.assertEqual(
            SCRIPT._select_context_operation(
                recall_available=True,
                maintenance_due=False,
                active_matches=False,
                allow_reactivation=True,
                nearest_generation_id=7,
            ),
            ("reactivate", 7, True),
        )
        self.assertEqual(
...
class WorkloadClockTests(unittest.TestCase):
    ...
    def test_protected_seed_sets_cannot_be_peeked_in_subsets(self) -> None:
        ...
        self.assertEqual(
            list(SCRIPT.SAME_CALL_CONFIRMATION_V1_SEEDS), expected_same_call
        )
        source = (
        ...
        self.assertEqual(derived, expected_same_call)
        with self.assertRaises(ValueError):
            SCRIPT._validate_split(
                "same_call_confirmation_v1", expected_same_call[:-1], False
            )
        SCRIPT._validate_split(
            "same_call_confirmation_v1", expected_same_call, False
        )
    ...
    def test_validation_v2_requires_formal_family_and_all_workloads(self) -> None:
        ...
        SCRIPT._validate_protocol_matrix(
            ...
            SCRIPT.SAME_CALL_CONFIRMATION_METHODS,
            SCRIPT.WORKLOADS,
            REGISTERED_METHODS,
        )
        with self.assertRaisesRegex(ValueError, "frozen same-call method family"):
            SCRIPT._validate_protocol_matrix(
                "same_call_confirmation_v1",
                tuple(reversed(SCRIPT.SAME_CALL_CONFIRMATION_METHODS)),
                SCRIPT.WORKLOADS,
                REGISTERED_METHODS,
            )
    ...
    def test_validation_v2_output_and_sortation_are_sealed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(
                FileExistsError, "same_call_confirmation_v1"
            ):
                SCRIPT._ensure_output_is_writable(
                    "same_call_confirmation_v1", existing
                )
