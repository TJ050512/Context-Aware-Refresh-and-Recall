...
def _split_protocol_metadata_b(split: str) -> dict[str, Any]:
    ...
    metadata.update({
        ...
        "seed_derivation": (
            ...
            f"{B_DERIVATION_SOURCE_SHA256} and domain="
            f"{B_DOMAIN} for i=0..39"
        ),
    })
    return metadata


def _bind_b_control_plane() -> None:
    """Bind the Experiment B roots and their non-causal provenance."""

    split_order = dict(V1.SPLIT_SEED_ORDER)
    split_order[SPLIT] = B_ROOTS
    split_sets = dict(V1.SPLIT_SEEDS)
    split_sets[SPLIT] = frozenset(B_ROOTS)
    V1.SAME_CALL_CONFIRMATION_V1_SEEDS = B_ROOTS
    V1.SPLIT_SEED_ORDER = split_order
    V1.SPLIT_SEEDS = split_sets
    V1._split_protocol_metadata = _split_protocol_metadata_b


def _bind_b_root_registry() -> None:
    """Backward-compatible name for the complete B control-plane binding."""

    _bind_b_control_plane()
