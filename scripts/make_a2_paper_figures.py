#!/usr/bin/env python3
"""Generate vector paper figures directly from the frozen A2 analysis JSON."""
...
ANALYSIS = ROOT / "reports" / "same_call_confirmation_a2_analysis.json"
OUT = ROOT / "output" / "pdf"
...
def make_pareto(data: dict) -> Path:
    path = OUT / "a2_pareto_frontier.pdf"
    width, height = 7.2 * inch, 4.45 * inch
    c = canvas.Canvas(str(path), pagesize=(width, height), pageCompression=1)
    c.setTitle("A2 sample-mean throughput-generator-call frontier")

    left, right, bottom, top = 58, 20, 46, 22
    pw, ph = width - left - right, height - bottom - top
    xmin, xmax = 0.8, 30.0
    ymin, ymax = 4390.0, 4620.0
    ...
    for tick in [4400, 4450, 4500, 4550, 4600]:
        y = sy(tick)
    ...
    offsets = {
        "bootstrap_only": (6, -12, "start"),
        "exact_even_G4": (-8, -14, "end"),
        "exact_even_G5": (7, -13, "start"),
        "random_G5": (7, -2, "start"),
        "js_cap_G5": (7, 5, "start"),
        "context_no_reactivation_B25": (9, 5, "start"),
...
def make_forest(data: dict) -> Path:
    path = OUT / "a2_superiority_forest.pdf"
    width, height = 7.2 * inch, 4.35 * inch
    c = canvas.Canvas(str(path), pagesize=(width, height), pageCompression=1)
    c.setTitle("A2 root-level superiority contrasts")

    left, right, bottom, top = 104, 80, 48, 36
    pw, ph = width - left - right, height - bottom - top
    xmin, xmax = -1.0, 5.0
    ...
    for method, y in zip(methods, ys):
        ...
        _text(c, width - right + 8, y - 3, f"{p_holm:.4f}", 7.5,
