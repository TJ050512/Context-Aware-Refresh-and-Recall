...
POLICY_DIR=${POLICY_DIR:-$REPO_ROOT/carr_rl_lite_exp/results/carr_rl_lite/ab}
OUT=${OUT:-$REPO_ROOT/carr_rl_lite_exp/results/carr_rl_lite/ab_train}
LOG=${LOG:-$REPO_ROOT/carr_rl_lite_exp/logs/carr_rl_lite/ab_train}
mkdir -p "$OUT" "$LOG"

# Six candidate (alpha, beta) policies, selected only on development seeds.
...
for cand in "${CANDS[@]}"; do
  for scen in narrow_r020 regular_r020; do
    read -r map agents <<< "${MAPS[$scen]}"
    "$PYTHON" "$DEV_RUNNER" \
      --workspace "$WORKSPACE" \
      --learned-policy-path "$POLICY_DIR/$cand.json" \
      --checkpoint "$CKPT" \
      ...
      --split development --seeds 17 18 19 20 \
      --workloads stationary abrupt \
      --methods context_learned_lite exact_even_B25 \
      ...
      --jobs 24 --fresh-process-per-arm \
      --map-path "$MAPDIR/$map" --agents "$agents" \
      --output "$OUT/${cand}__${scen}.json" \
      > "$LOG/${cand}__${scen}.log" 2>&1 &
  done
done
wait
echo TRAIN_DONE
