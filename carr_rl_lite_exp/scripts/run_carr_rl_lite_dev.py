...
SPLIT_SEED_ORDER = {
    ...
    "same_call_confirmation_v1": SAME_CALL_CONFIRMATION_V1_SEEDS,
}
SPLIT_SEEDS = {
...
COMPLETE_SEED_SPLITS = frozenset(
    {
        "contaminated_pilot_validation",
        "validation_v2",
        "locked_test",
        "same_call_confirmation_v1",
    }
)
FORMAL_FAMILY_SPLITS = frozenset(
    {"contaminated_pilot_validation", "validation_v2", "locked_test"}
)
DEVELOPMENT_STYLE_SPLITS = frozenset(
    {"development", "same_call_confirmation_v1"}
)
SAME_CALL_CONFIRMATION_METHODS = (
    "bootstrap_only",
    "exact_even_G4",
    "exact_even_G5",
    "random_G5",
...
OVERWRITE_PROTECTED_SPLITS = COMPLETE_SEED_SPLITS
...
def _attempt_identity(spec: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "split": str(spec["split"]),
        "map_id": Path(spec["map_path"]).stem,
...
def _append_attempt_event(path: Path, payload: Mapping[str, Any]) -> None:
...
def _select_context_operation(
    *,
    recall_available: bool,
    maintenance_due: bool,
    active_matches: bool,
    allow_reactivation: bool,
    nearest_generation_id: int | None,
) -> tuple[str, int | None, bool]:
    """Resolve the causal context action before policy guards are applied.

    The no-reactivation ablation preserves the base priority order but treats
    historical recall as unavailable.  It does not map a recall directly to a
    generation; after removing that branch, maintenance and active-context
    matching retain their original precedence.
    """
    ...
    if recall_available and allow_reactivation:
        if nearest_generation_id is None:
            raise ValueError("available recall requires a generation id")
        return "reactivate", int(nearest_generation_id), True
    if maintenance_due:
...
def _permute_future_suffix(
    ...
    *,
    ...
    """Rotate only same-type future goals for the metamorphic preflight."""

    if cutoff <= 0 or variant <= 0:
        raise ValueError("future-suffix cutoff and variant must be positive")
    changed = 0
    for agent_id, sequence in enumerate(tape):
        by_type: dict[str, list[int]] = {}
        for tape_index, (goal, release) in enumerate(sequence):
            if int(release) >= cutoff:
                by_type.setdefault(grid_types[int(goal)], []).append(tape_index)
        for goal_type, indices in sorted(by_type.items()):
            if len(indices) < 2:
                continue
            shift = 1 + _keyed_uint32(
                "future-suffix-metamorphic",
                seed,
                variant,
                agent_id,
                goal_type,
            ) % (len(indices) - 1)
            original = [int(sequence[index][0]) for index in indices]
            rotated = original[shift:] + original[:shift]
            for index, replacement in zip(indices, rotated):
                changed += int(sequence[index][0] != replacement)
                sequence[index][0] = replacement
    return changed
...
def _build_manifest(spec: Mapping[str, Any]) -> tuple[dict[str, Any], Any, Path]:
    ...
    suffix_variant = int(spec.get("future_suffix_variant", 0))
    suffix_cutoff = int(spec.get("future_suffix_cutoff", total_steps))
    if suffix_variant:
        changed = _permute_future_suffix(
            tape,
            grid_types=kiva_map.grid_types,
            cutoff=suffix_cutoff,
            seed=seed,
            variant=suffix_variant,
        )
        if changed == 0:
            raise RuntimeError("future-suffix variant did not change any task goal")
    ...
    if suffix_variant:
        generator_metadata.update({
            ...
            "future_suffix_cutoff_absolute": suffix_cutoff,
            "future_suffix_metamorphic_only": True,
        })
...
def _run_one(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _pin_torch_threads()
    from dai_lmapf.same_call_claim_runner import (
    ...
    precommitted_post_bootstrap_schedule = (
        [int(epoch) + 1 for epoch in sorted(policy._schedule)]
        if policy is not None and hasattr(policy, "_schedule")
        else None
    )
        ...
        if decision_index == 0:
            ...
            if method == "causal_block_B25":
                ...
                (
                    ...
                    target_generation_id,
                    action_available,
                ) = _select_context_operation(
                    recall_available=recall_available,
                    maintenance_due=maintenance_due,
                    active_matches=active_matches,
                    allow_reactivation=(
                        method != CONTEXT_NO_REACTIVATION_METHOD
                    ),
                    nearest_generation_id=nearest_generation_id,
                )
                ...
                if method == CONTEXT_DUALCAP_METHOD:
                    ...
                    action_available = proposed_operation != "hold"
                if method == "context_learned_lite":
    ...
    if method == CONTEXT_NO_REACTIVATION_METHOD and (
        post_reactivations != 0 or post_publications != post_generations
    ):
        raise RuntimeError(
            "no-reactivation ablation executed a recall or mismatched switches"
        )
    ...
    result = {
        ...
        "execution_parent_process_id": os.getppid(),
        "execution_host": socket.gethostname(),
        "execution_process_start_ns": process_started_ns,
        "run_uuid": run_uuid,
        "seed": seed,
        "root_seed": seed,
        "split": spec["split"],
        ...
        "budget": {
            ...
            "budget_semantics": (
                ...
                else "at_most" if method in {"js_cap_B25", "js_cap_G5"}
...
def _split_protocol_metadata(split: str) -> dict[str, Any]:
    ...
    elif split == "same_call_confirmation_v1":
        metadata.update({
            "classification": "fresh_post_hoc_same_call_confirmation",
            "seed_derivation": (
                ...
                "and domain=dai-same-call-confirmatory-v1 for i=0..9"
            ),
...
def _validate_split(split: str, seeds: Iterable[int], confirm_locked: bool) -> None:
    ...
    if split in COMPLETE_SEED_SPLITS and set(seeds) != set(
        SPLIT_SEEDS[split]
    ):
        raise ValueError(
...
def _validate_protocol_matrix(
    ...
    split = _canonical_split(split)
    if split in FORMAL_FAMILY_SPLITS:
        if set(methods) != set(registered_methods):
            raise ValueError(
                f"{split} must include the complete registered method family"
            )
    elif split == "same_call_confirmation_v1":
        if tuple(methods) != SAME_CALL_CONFIRMATION_METHODS:
            raise ValueError(
                "same_call_confirmation_v1 must include the frozen same-call "
                "method family in its declared order"
            )
    else:
        return
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "append-only JSONL attempt ledger; mandatory for the fresh "
            "same-call confirmation split"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--future-suffix-variant",
        type=int,
        default=0,
        help="development-only metamorphic suffix permutation identifier",
    )
    parser.add_argument(
        "--future-suffix-cutoff",
        type=int,
        default=None,
        help="absolute timestep at which the development-only suffix begins",
    )
    parser.add_argument(
        "--fresh-process-per-arm",
        action="store_true",
        help=(
            "run every method arm in a newly spawned OS process; this is "
            "mandatory for same_call_confirmation_v1 and available for smoke"
        ),
    )
    parser.add_argument(
        "--exclusive-timing",
    ...
    if args.development_matrix_config_sha256 is not None:
        ...
        if args.split not in DEVELOPMENT_STYLE_SPLITS:
            raise ValueError(
                "a development-matrix config digest is valid only for a "
                "development-style split"
            )
    ...
    if args.split == "same_call_confirmation_v1" and args.attempt_ledger is None:
        raise ValueError("same_call_confirmation_v1 requires --attempt-ledger")
    ...
    if args.future_suffix_variant < 0:
        raise ValueError("--future-suffix-variant must be non-negative")
    if args.future_suffix_variant and args.split != "development":
        raise ValueError("future-suffix variants are permitted only on development")
    total_absolute_steps = args.warmup_time + args.horizon
    if args.future_suffix_cutoff is None:
        args.future_suffix_cutoff = total_absolute_steps
    if not 0 < args.future_suffix_cutoff < total_absolute_steps:
        if args.future_suffix_variant:
            raise ValueError("future-suffix cutoff must lie inside the run horizon")
    ...
    if attempt_ledger is not None:
        if attempt_ledger.exists():
            raise FileExistsError(
                f"refusing to overwrite append-only attempt ledger: {attempt_ledger}"
            )
    ...
    runtime_source_paths = {
        "claim_runner": workspace / "src/dai_lmapf/same_call_claim_runner.py",
        "validation_runner": Path(__file__).resolve(),
        "publication_policy": workspace / "src/dai_lmapf/publication_policy.py",
        "frozen_cnn_generator": workspace / "src/dai_lmapf/frozen_cnn_generator.py",
        "online_ggo_adapter": workspace / "src/dai_lmapf/online_ggo_adapter.py",
        "trafficflow_online_env": (
            cmaes / "env_search/iterative_update/envs/trafficflow_online_env.py"
        ),
        "task_generator": cmaes / "env_search/utils/task_generator.py",
    ...
    if missing_runtime_sources:
        raise FileNotFoundError(
            "runtime source manifest is incomplete: "
    ...
    common = {
        ...
        "future_suffix_variant": args.future_suffix_variant,
        "future_suffix_cutoff": args.future_suffix_cutoff,
    ...
    if attempt_ledger is not None:
        for spec in work:
            identity = _attempt_identity(spec)
            _append_attempt_event(attempt_ledger, {
                "schema": ATTEMPT_LEDGER_SCHEMA,
                "event": "started",
                "attempt_id": _strict_json_sha256(identity),
                "identity": identity,
                "runner_parent_process_id": os.getpid(),
                "time_ns": time.time_ns(),
            })

    def record_completion(run: Mapping[str, Any]) -> None:
        ...
        _append_attempt_event(attempt_ledger, {
            ...
            "identity": identity,
            "runner_parent_process_id": os.getpid(),
            "execution_process_id": int(run["execution_process_id"]),
            "execution_host": str(run["execution_host"]),
            "execution_process_start_ns": int(
                run["execution_process_start_ns"]
            ),
            "run_uuid": str(run["run_uuid"]),
            "run_sha256": _strict_json_sha256(run),
            "planner_timeouts": int(run["planner_timeouts"]),
            "safety_passed": run["safety"]["passed"] is True,
            "invariants_passed": run["invariants"]["passed"] is True,
            "time_ns": time.time_ns(),
        })

    runs: list[dict[str, Any]] = []
    fresh_process_per_arm = bool(
        args.fresh_process_per_arm
        or args.split == "same_call_confirmation_v1"
    )
    if fresh_process_per_arm:
        # The confirmatory protocol requires every arm to start in a fresh OS
        # process so Python/C++ static state cannot leak across methods.  The
        # Python 3.9 runtime lacks ProcessPoolExecutor(max_tasks_per_child),
        # therefore use multiprocessing.Pool's equivalent contract here.
        context = multiprocessing.get_context("spawn")
        with context.Pool(
            processes=args.jobs,
            maxtasksperchild=1,
        ) as pool:
            for run in pool.imap_unordered(_run_one, work, chunksize=1):
                runs.append(run)
    ...
    attempt_ledger_record = None
    if attempt_ledger is not None:
        attempt_ledger_record = {
            "schema": ATTEMPT_LEDGER_SCHEMA,
            "path": str(attempt_ledger),
            "sha256": _sha256_file(attempt_ledger),
            "started_events": len(work),
    ...
    artifact = {
        ...
        "fresh_process_per_arm": fresh_process_per_arm,
        "attempt_ledger": attempt_ledger_record,
        ...
        "protocol": {
            ...
            "future_suffix_variant": args.future_suffix_variant,
            "future_suffix_cutoff_absolute": args.future_suffix_cutoff,
            "sigma": args.sigma,
            "pacing_slack": args.pacing_slack,
            "minimum_history": args.minimum_history,
            "causal_block_B25": {
            ...
            "context_no_reactivation_B25": {
                ...
                "ablation_rule": (
                    "treat historical recall as unavailable, then preserve "
                    "maintenance-before-active-match-before-generate priority "
                    "and all causal policy guards"
                ),
                "score": "release_only_4x4_fast2_vs_prior6_js",
                "score_quantile": 0.75,
                "absolute_score_gate": args.context_min_score,
                ...
                "minimum_effect_windows": 3,
                "context": "causal_active_goal_4x4",
                "post_bootstrap_generation_and_switch_cap": (
                    math.ceil(0.25 * (args.horizon // args.decision_window - 1))
                ),
